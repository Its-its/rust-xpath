...
// macro_rules! test {
// 	($value:expr) => {{
// 		let tokenizer = parser::Tokenizer::new($value);
// 		let found = tokenizer.collect::<Vec<parser::TokenResult>>();

// 		if found.iter().find(|i| i.is_err()).is_some() {
// 			println!("Err('{}'): {:?}", $value, found);
// 		} else {
// 			println!("Ok('{}'): {:?}", $value, found.into_iter().map(|i| i.unwrap()).collect::<Vec<_>>());
// 		}
// 	}};
// }
...
pub fn parse_url<S: ToString>(url: S) -> Document {
	let parse: markup5ever_rcdom::RcDom = html5ever::parse_document(markup5ever_rcdom::RcDom::default(), Default::default())
		.from_utf8()
		.read_from(&mut File::open("./doc/example.html").expect("File::open"))
		.expect("html5ever");
...
fn main() {
	...
	// dbg!(doc.evaluate("//span"));
	dbg!(doc.evaluate("//@href"));

	println!("Location Paths (Unabbreviated Syntax)");
	// dbg!(doc.evaluate("/")); // selects the document root (which is always the parent of the document element)
