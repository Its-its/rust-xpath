...
mod tests {
    ...
    fn assert_is_error(doc: &Document, search: &str) {
        assert_eq!(
            evaluate(doc, search).map(|v| v.is_err()),
            Some(true),
            "IS ERR {:?}",
            search
        );
    ...
    fn assert_is_ok(doc: &Document, search: &str) {
        assert_eq!(
            evaluate(doc, search).map(|v| v.is_ok()),
            Some(true),
            "IS OK {:?}",
            search
        );
    ...
    fn assert_eq_eval_to_string<I: ToString>(doc: &Document, search: &str, value: I) {
        assert_eq!(
            evaluate(doc, search).map(|v| v.and_then(|v| v.convert_to_string())),
    ...
    fn general_examples() {
        ...
        assert_eq_eval(
            &doc,
            r#"//div[contains(text(), "Testing 1")]/@class"#,
